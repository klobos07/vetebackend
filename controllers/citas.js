...
const getCitas = async (req, res = response) => {
  const citas = await Cita.find({medico: req.params.id})
...
const getCitasByUser = async (req, res = response) => {
...
const actualizarCitas = async (req, res = response) => {
  ...
  try {
    ...
    const citaActualizado = await Cita.findByIdAndUpdate(id, cambiosCita, {
      new: true,
    });
